...
- [x] 决定主 canonical 域名：Cloudflare Pages，全站已加 canonical / og:url。
...
- [ ] **添加 404 页面**（已升级为高优先级）：Pages 没有 `404.html` 时，任意不存在的路径都返回 200 + 首页内容，构成 soft 404。

## 已完成

- [x] 修好 `scripts/check_site.py` 跨天必然报 sitemap stale 的问题（`date.today()` → `SITE_LASTMOD`）。
